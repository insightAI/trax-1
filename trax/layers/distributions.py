...
class Distribution:
  ...
  def param_shape(self):
    """Returns the shape of distribution parameters."""
  ...
  def sample(self, params):
    ...
    Args:
      params (np.ndarray): Distribution parameters. Shape is
        subclass-specific. Broadcasts along the first dimensions. For example,
        in the categorical distribution parameter shape is (C,), where C is the
        number of categories. If (B, C) is passed, the object will represent
        a batch of B categorical distributions with different parameters.
  ...
  def log_prob(self, params, point):
  ...
  def LogProbLayer(self):  # pylint: disable=invalid-name
    ...
    def Layer(x, **unused_kwargs):  # pylint: disable=invalid-name
      ...
      (params, point) = x
      return self.log_prob(params, point)
...
class CategoricalDistribution(Distribution):
  ...
  def sample(self, params):
    return core.gumbel_sample(params)

  def log_prob(self, params, point):
    # Flatten the prefix dimensions for easy indexing.
    flat_point = np.reshape(point, -1)
    flat_params = np.reshape(params, (point.size, -1))
    flat_log_probs = flat_params[np.arange(point.size), flat_point.astype(int)]
...
def create_distribution(space):
  ...
  if isinstance(space, gym.spaces.Discrete):
    return CategoricalDistribution(space.n)
  else:
